...
def generate_costmap(mask_dict, t_dict={"t_l":0.4, "t_a":0.6}, device="cpu"):
    ...
    # Unknown Mask
    mask_count = road_mask.to(torch.float32) + trail_mask.to(torch.float32) + grass_mask.to(torch.float32) +                 buildings_mask.to(torch.float32) + trees_mask.to(torch.float32) + water_mask.to(torch.float32)
    ...
    costmap[grass_mask] += 1000 * grass_logit[grass_mask]
    costmap[buildings_mask] += 1000 * buildings_logit[buildings_mask]
    costmap[trees_mask] += 1000 * trees_logit[trees_mask]
    costmap[water_mask] += 1000 * water_logit[water_mask]
