...
def generate_costmap(mask_dict, t_dict={"t_l":0.4, "t_a":0.6}):
            ...
            railway_tracks_logit = mask_dict.get('railway tracks', torch.zeros(shape, dtype=torch.float32, device=device))
            ...
            railway_tracks_logit = torch.from_numpy(railway_tracks_logit).to(device)
            ...
            railway_tracks_mask = railway_tracks_logit > t_a
            ...
            # Unknown Mask
            mask_count = road_mask.to(torch.float32) + trail_mask.to(torch.float32) + grass_mask.to(torch.float32) +                         buildings_mask.to(torch.float32) + trees_mask.to(torch.float32) + water_mask.to(torch.float32)                         + railway_tracks_mask.to(torch.float32)
            ...
            costmap[railway_tracks_mask] += 1000 * railway_tracks_logit[railway_tracks_mask]
